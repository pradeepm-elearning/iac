...
| `terraform output <<RESOURCE_NAME>>` | prints the output of a specific variable. |
...
| `terraform providers mirror <<PATH_OF_OTHER_FOLDER>>` | copies provider plugins needed for the current configuration to another directory. <br> eg: `terraform providers mirror /tmp/terraform_files/` |
| `terraform output` | prints all the output variables in the configuration directory. |
| `terraform output <<VARIABLE>>` | print the value of the specific variable by appending the name of the variable to the end of the output command. <br> eg: `terraform output pet-name` |
